...
void deleteNucleotides(Nucleotide* dna) {
    ...
    Nucleotide* a = dna->next;
    Nucleotide* b = dna->prev;

    delete dna;

    while (a != nullptr){
        Nucleotide* next = a->next;
        delete a;
        a = next;
    }

    while (b != nullptr){
        Nucleotide* prev = b->prev;
        delete b;
        b = prev;
    }
}

void deleteNext(Nucleotide* dna){
    if (dna == nullptr) {return;}
...
void deletePrev(Nucleotide* dna){
    if (dna == nullptr) {return;}

    Nucleotide* a = dna->prev;
...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide* head = new Nucleotide;
    Nucleotide* Prev;
    Prev = head;

    for (char i : str){
        Nucleotide* now = new Nucleotide;
        now->value = i;
        now->prev = Prev;
        now->next = nullptr;
        Prev->next = now;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    bool flag = false;
    Nucleotide* st = findFirst(dna, target);

    if (st == nullptr) return flag;
    else flag = true;

    if (st == dna){
        while(target){
            Nucleotide* t = st->next;
            delete st;
            st = t;
            target = target->next;
        }
        dna = st;
        if (dna) dna->prev = nullptr;
    }else{
        Nucleotide* Prev = st->prev;
        while(target){
            Nucleotide* t = st->next;
            delete st;
            st = t;
