...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            ...
                            double height) {
    int m = terrain.numRows(), n = terrain.numCols();
    Grid<bool> ans(m, n);
    ...
    while (!q.isEmpty()){
        GridLocation source = q.dequeue();
        int i = source.row;
        int j = source.col;
        for (int a = 0; a< 4; a++){
            if (terrain.inBounds(i+x[a], j+y[a]) && terrain[i+x[a]][j+y[a]] <= height && ans[i+x[a]][j+y[a]] == false){
                q.add(GridLocation(i+x[a], j+y[a]));
                ans[i+x[a]][j+y[a]] = true;
            }
        }
    }
