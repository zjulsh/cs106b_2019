...
bool canAllPatientsBeSeen(const HashMap<string, int>& doctors,
    ...
    HashMap<string, int> Doctors = doctors;
    HashMap<string, int> Patients = patients;

    return help(Doctors, Patients, schedule);
}

bool help(HashMap<string, int>& doctors,
    ...
    int t = patients[patient];
    Vector<string> doctorName = doctors.keys();
    for (auto name : doctorName){
        int time = doctors[name];
        if (time >= t){
            schedule[name].add(patient);
            doctors[name] -= t;
            patients.remove(patient);
            if (help(doctors, patients, schedule)) return true;
            else{
                patients.add(patient, t);
                schedule[name].remove(patient);
                doctors[name] += t;
            }
        }
    }
