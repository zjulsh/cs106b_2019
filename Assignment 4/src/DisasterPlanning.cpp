...
bool canBeMadeDisasterReady(const HashMap<string, HashSet<string>>& roadNetwork,
    ...
    if (numCities < 0){
        error("Please input positive number of numCities!");
        return false;
    }

    //HashMap<string, HashSet<string>> road = roadNetwork;
    //Vector<string> remain = road.keys();

    return help(roadNetwork, numCities, supplyLocations);
}


bool help(const HashMap<string, HashSet<string>>& roadNetwork,
          int numCities,
          HashSet<string>& supplyLocations){
    if (judge(roadNetwork, supplyLocations)) return true;
    if (numCities == 0) return false;

    //Vector<string> roadName = roadNetwork.keys();
    HashSet<string> roadName;
    for (auto i : roadNetwork){
        roadName.add(i);
    }
    roadName -= supplyLocations;
    for (auto i : roadName){
        if (!judgeNeighbor(i, roadNetwork, supplyLocations)){
            numCities--;
            supplyLocations.add(i);
            if (help(roadNetwork, numCities, supplyLocations)) return true;
            supplyLocations.remove(i);
            for (auto j : roadNetwork[i]){
                supplyLocations.add(j);
                if (help(roadNetwork, numCities, supplyLocations)) return true;
                supplyLocations.remove(j);
            }
            numCities++;
        }
    }
...
bool judge(const HashMap<string, HashSet<string>>& roadNetwork,
           HashSet<string>& supplyLocations){
    HashMap<string, HashSet<string>> remain = roadNetwork;

    for (string i : supplyLocations){
        remain.remove(i);
        for (string j: roadNetwork[i]){
            remain.remove(j);
        }
    }
...
bool judgeNeighbor(string name,
                   const HashMap<string, HashSet<string>>& roadNetwork,
                   HashSet<string>& supplyLocations){
    if (supplyLocations.contains(name)) return true;

    for (string i : supplyLocations){
        if (roadNetwork[i].contains(name)) return true;
    }
