...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if (sequences.size() == 1) {
        Vector<DataPoint> ans = sequences[0];
        return ans;
    }

    int k = sequences.size();
    Vector<DataPoint> left = combine(sequences.subList(0, k/2));
    Vector<DataPoint> right = combine(sequences.subList(k/2, k - k/2));

    Vector<DataPoint> merge;
    int i = 0, j = 0;
    ...
    if (i < left.size()){
        merge += left.subList(i);
    }
    else{
        merge += right.subList(j);
    }
